...
try:
    ...
    print("✅ YouTube module connected")
except ImportError:
...
# WhatsAppTask import
try:
    from modules.app_control.whatsapp_task import WhatsAppDesktopEngine
...
class SystemControl:
    def __init__(self):
        ...
        # WhatsApp instance
        self.whatsapp = WhatsAppDesktopEngine() if WHATSAPP_AVAILABLE else None
    ...
    def handle(self, cmd: str):
        cmd = cmd.lower().strip()
        print(f"[DEBUG] Processing: '{cmd}'")  # Debug line
        ...
        if self.awaiting_power_confirmation:
            ...
            if cmd in ["no", "cancel", "nahi", "stop"]:
                self.awaiting_power_confirmation = None
                return "Power action cancel kar diya boss."
        ...
        if self.awaiting_close_all_confirmation:
            ...
            if cmd in ["no", "cancel", "nahi", "stop"]:
                self.awaiting_close_all_confirmation = False
                return "Close all action cancel kar diya boss."
            return "Boss sirf Yes ya No bolo."

         # ================= WHATSAPP FIXED LOGIC =================
        if "ko" in cmd and self.whatsapp:
            words = cmd.split()

            if "ko" in words:
                ko_index = words.index("ko")

                # Contact = everything before "ko"
                contact = " ".join(words[:ko_index]).strip()

                # Message = everything after "ko"
                message = " ".join(words[ko_index + 1:]).strip()

                if contact and message:
                    print(f"[WhatsApp] Triggered: {contact} <- '{message}'")
                    return self.whatsapp.send_message(contact, message)

        # Pattern: send message to X saying Y
        if cmd.startswith("send message tu ") and self.whatsapp:
            try:
                content = cmd.replace("send message tu ", "")
                parts = content.split(" saying ")
                contact = parts[0].strip()
                message = parts[1].strip()
                return self.whatsapp.send_message(contact, message)
            except:
                return "Format bolo: send message to abc saying xyz"
        ...
        close_commands = ["close youtube", "close youtube app", "close youtube browser"]
        if any(c in cmd for c in close_commands) and self.youtube:
            return self.youtube.execute("close")
    ...
    def battery_status(self):
        ...
        if b:
            return f"Battery Status {b.percent}% boss."
